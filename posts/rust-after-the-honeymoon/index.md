...
And we can set `RUST_BACKTRACE` to get a full backtrace where an error actually originates -- which is especially useful when a failure emerges from a surprising place, like this one from a Drop implementation in [probe-rs](https://github.com/probe-rs/probe-rs):
...
As an aside, [`format_args!`](https://doc.rust-lang.org/std/macro.format_args.html) is really magical -- and a subject that really merits its own blog post from someone with more expertise on the subject. (Yes, this is the Rust blogging equivalent of Chekhov's gun!)
